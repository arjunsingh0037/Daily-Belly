 ...
 * @return string
 */
function theme_brink_get_extra_scss($theme) {
    $scss = $theme->settings->scss;

    $scss .= theme_brink_set_headerimg($theme);
 ...
 * @return string
 */
function theme_brink_set_headerimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_brink_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_brink', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_brink and not theme_boost (see the line above).
    ...
    // Brink scss.
    $brink = file_get_contents($CFG->dirroot . '/theme/brink/scss/brink.scss');
 ...
 * @return string
 */
function theme_brink_get_pre_scss($theme) {
    global $CFG;
 ...
 * @return mixed
 */
function theme_brink_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM and $filearea === 'logo') {
        $theme = theme_config::load('brink');
    ...
    }else if ($context->contextlevel == CONTEXT_SYSTEM and $filearea === 'categoryimg') {
        $theme = theme_config::load('brink');
        return $theme->setting_file_serve('categoryimg', $args, $forcedownload, $options);
 ...
 * @return string
 */
function theme_brink_get_setting($setting, $format = false) {
    $theme = theme_config::load('brink');
 ...
 * @return array
 */
function theme_brink_get_marketing_items() {
    global $PAGE, $OUTPUT;
    ...
    if (!empty($PAGE->theme->settings->marketing1heading)) {
        $templatecontext['marketing1heading'] = theme_brink_get_setting('marketing1heading', true);
    }

    $templatecontext['marketing1subheading'] = '';
    if (!empty($PAGE->theme->settings->marketing1subheading)) {
        $templatecontext['marketing1subheading'] = theme_brink_get_setting('marketing1subheading', true);
    }

    $templatecontext['marketing1content'] = '';
    if (!empty($PAGE->theme->settings->marketing1content)) {
        $templatecontext['marketing1content'] = theme_brink_get_setting('marketing1content', true);
    }
    ...
    if (!empty($PAGE->theme->settings->marketing2heading)) {
        $templatecontext['marketing2heading'] = theme_brink_get_setting('marketing2heading', true);
    }

    $templatecontext['marketing2subheading'] = '';
    if (!empty($PAGE->theme->settings->marketing2subheading)) {
        $templatecontext['marketing2subheading'] = theme_brink_get_setting('marketing2subheading', true);
    }

    $templatecontext['marketing2content'] = '';
    if (!empty($PAGE->theme->settings->marketing2content)) {
        $templatecontext['marketing2content'] = theme_brink_get_setting('marketing2content', true);
    }
    ...
    if (!empty($PAGE->theme->settings->marketing3heading)) {
        $templatecontext['marketing3heading'] = theme_brink_get_setting('marketing3heading', true);
    }

    $templatecontext['marketing3subheading'] = '';
    if (!empty($PAGE->theme->settings->marketing3subheading)) {
        $templatecontext['marketing3subheading'] = theme_brink_get_setting('marketing3subheading', true);
    }

    $templatecontext['marketing3content'] = '';
    if (!empty($PAGE->theme->settings->marketing3content)) {
        $templatecontext['marketing3content'] = theme_brink_get_setting('marketing3content', true);
    }
    ...
    if (!empty($PAGE->theme->settings->marketing4heading)) {
        $templatecontext['marketing4heading'] = theme_brink_get_setting('marketing4heading', true);
    }

    $templatecontext['marketing4subheading'] = '';
    if (!empty($PAGE->theme->settings->marketing4subheading)) {
        $templatecontext['marketing4subheading'] = theme_brink_get_setting('marketing4subheading', true);
    }

    $templatecontext['marketing4content'] = '';
    if (!empty($PAGE->theme->settings->marketing4content)) {
        $templatecontext['marketing4content'] = theme_brink_get_setting('marketing4content', true);
    }
...
function get_category_content(){
    global $PAGE;
    $content = '';
    if (!empty($PAGE->theme->settings->categoryimg)) {
        $content .= html_writer::start_tag('div', array('class' => 'catsettings'));
        $headerimg = $PAGE->theme->setting_file_url('categoryimg', 'categoryimg');
        $content .= '<img src="'.$headerimg.'" alt="Category Header Image">';
        $content .= html_writer::end_tag('div');
    }
    if (!empty($PAGE->theme->settings->categorycontent)) {
        $content .= html_writer::start_tag('div', array('class' => 'catsettings_text'));
        $content .= html_writer::tag('p',$PAGE->theme->settings->categorycontent);
        $content .= html_writer::end_tag('div');
    }
    return $content;
}